<?php
	...
	$characterID = !empty($_GET['characterID']) ? (int)htmlspecialchars($_GET['characterID']) : $characterID;
	$keyID = !empty($_GET['keyID']) ? (int)htmlspecialchars($_GET['keyID']) : 0;
	$vCode = !empty($_GET['vCode']) ? htmlspecialchars($_GET['vCode']) : "";
	...
	if (!empty($characterID)) {
		...
		if (empty($keyID) && empty($vCode))
		{
			...
			$vCode = (int) mysql_result($result, 0, 'vCode');
		}
...
<?php echo getPageselection($title, is_numeric($characterID) && $characterID != 0 ? "//image.eveonline.com/Character/".$characterID."_64.jpg" : "//image.eveonline.com/Type/2014_64.png"); ?>
		<div id="content">
<?php
//			loginwithapi();
